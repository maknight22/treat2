...
shinyServer(function(input, output) {
  ...
  iv$add_rule("age", sv_numeric())
  iv$add_rule("gender", sv_required())
  iv$add_rule("packs", sv_numeric())
  iv$add_rule("size", sv_numeric())
  iv$add_rule("upperlobe", sv_required())
  #iv$enable()
  ...
  observeEvent(input$submit,{
    if(iv$is_valid()){
      ...
      output$result <- renderTable({
        ...
        #data1.gender = ifelse(data$gender=="Male", 1, data$gender)
        #data1.gender = ifelse(data$gender=="Female", 0, data$gender)
        #data1.gender = ifelse(data$gender=="Missing", NA, data$gender)
        if(data$gender=="Male"){
          data1.gender = 1
        ...
        #data1.spicul = ifelse(data$spicul=="Yes", 1, data$spicul)
        #data1.spicul = ifelse(data$spicul=="No", 0, data$spicul)
        #data1.spicul = ifelse(data$spicul=="Missing", NA, data$spicul)
        if(data$spicul=="Yes"){
          data1.spicul = 1
        ...
        #data1.upperlobe = ifelse(data$upperlobe=="Yes", 1, data$upperlobe)
        #data1.upperlobe = ifelse(data$upperlobe=="No", 0, data$upperlobe)
        #data1.upperlobe = ifelse(data$upperlobe=="Missing", NA, data$upperlobe)
        ...
        #data1.prev_cancer = ifelse(data$prev_cancer=="Yes", 1, data$prev_cancer)
        #data1.prev_cancer = ifelse(data$prev_cancer=="No", 0, data$prev_cancer)
        #data1.prev_cancer = ifelse(data$prev_cancer=="Missing", NA, data$prev_cancer)
        ...
        #data1.anysympt = ifelse(data$anysympt=="Yes", 1, data$anysympt)
        #data1.anysympt = ifelse(data$anysympt=="No", 0, data$anysympt)
        #data1.anysympt = ifelse(data$anysympt=="Missing", NA, data$anysympt)
        ...
        #data1.petavid = ifelse(data$petavid=="Yes", 1, data$petavid)
        #data1.petavid = ifelse(data$petavid=="No", 0, data$petavid)
        #data1.petavid = ifelse(data$petavid=="Missing", NA, data$petavid)
        if(data$petavid == "Yes"){
          data1.petavid = 1
        ...
        #data1.group = 
        #  ifelse(data$group=="Pulmonary Nodule Clinic", 1, data$group)
        #data1.group = 
        #  ifelse(data$group=="Thoracic Surgery Clinic", 2, data$group)
        #data1.group = 
        #  ifelse(data$group=="Surgical Resection", 3, data$group)
        #data1.group =
        #  ifelse(data$group=="Missing", NA, data$group)
        ...
        #draw table
        resultTable = data.frame("Probability" = paste0(round(prob*100, 1),"%"),
                                 "CI" = paste0("(",round(prob.lowerbound*100,1),"%, ",round(prob.upperbound*100,1),"%)"))
        resultTable
